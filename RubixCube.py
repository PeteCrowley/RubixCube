...
class RubixCube:
    ...
    def rotate_counter_clockwise(self, face: int):
        ...
        if face == 7:
            temp = [self.cube[5][i][1] for i in range(self.length)]
            for i in range(self.length):
                self.cube[5][i][1] = self.cube[1][i][1]
                self.cube[1][i][1] = self.cube[4][i][1]
                self.cube[4][i][1] = self.cube[3][self.length - 1 - i][1]
                self.cube[3][self.length - 1 - i][1] = temp[i]
        elif face == 8:
            temp = [self.cube[0][1][i] for i in range(self.length)]
            for i in range(self.length):
                self.cube[0][1][i] = self.cube[1][1][i]
                self.cube[1][1][i] = self.cube[2][1][i]
                self.cube[2][1][i] = self.cube[3][1][i]
                self.cube[3][1][i] = temp[i]
        else:
            for i in range(3):
        ...
        if face == -1:
            ...
            for x in range(0, int(self.length / 2)):
                ...
                for y in range(x, self.length - x - 1):
                    ...
                    mat[x][y] = mat[self.length - 1 - y][x]
                    # move values from bottom to right
                    mat[self.length - 1 - y][x] = mat[self.length - 1 - x][self.length - 1 - y]
                    # move values from left to bottom
                    mat[self.length - 1 - x][self.length - 1 - y] = mat[y][self.length - 1 - x]
                    # assign temp to left
                    mat[y][self.length - 1 - x] = temp
        elif face < 6:
            mat = self.cube[face]
            for x in range(0, int(self.length / 2)):
                # Consider elements in group
